...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        #splash-screen.hidden { opacity: 0; pointer-events: none; transform: scale(1.1); }
        .splash-logo { width: 80%; max-width: 400px; object-fit: contain; animation: breathe 3s infinite; }
        @keyframes breathe { 0%, 100% { transform: scale(0.98); opacity: 0.9; } 50% { transform: scale(1.02); opacity: 1; } }
        .click-hint { position: absolute; bottom: 100px; color: #999; font-size: 1.2rem; animation: blink 2s infinite; }
        ...
        .modal-header { padding: 15px; background: #333; color: white; display: flex; justify-content: space-between; align-items: center; font-size: 1.1rem; font-weight: bold; }
        .modal-body { padding: 15px; overflow-y: auto; flex: 1; line-height: 1.6; }
        .modal-footer { padding: 15px; border-top: 1px solid #eee; }
        
        /* 食譜步驟列表 */
        .step-list { padding-left: 20px; margin: 0; color: #444; }
...
<body>
    ...
    <div id="cart-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><span>購物車</span><span onclick="closeModal('cart-modal')" style="cursor:pointer;">✕</span></div>
    ...
    <div id="book-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header"><span>我的收藏食譜</span><span onclick="closeModal('book-modal')" style="cursor:pointer;">✕</span></div>
    ...
    <div id="steps-modal" class="modal">
        <div class="modal-content">
            <div class="modal-header">
                <span id="steps-title">料理做法</span>
                <span onclick="closeModal('steps-modal')" style="cursor:pointer;">✕</span>
            </div>
            <div class="modal-body" id="steps-body"></div>
            <div class="modal-footer">
                <button class="btn-outline" style="width:100%;" onclick="closeModal('steps-modal')">關閉</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let recipes = [
            { id: "R1", name: "綜合蔬果沙拉", cal: 220, img: "images/綜合蔬果沙拉.jpg", ingredients: ["蘋果", "番茄", "洋蔥"], steps: ["所有食材洗淨切塊", "加入橄欖油與鹽拌勻", "撒上些許胡椒提味"] },
            { id: "R2", name: "番茄炒高麗菜", cal: 180, img: "images/番茄炒高麗菜.jpg", ingredients: ["番茄", "高麗菜"], steps: ["熱鍋爆香蒜末", "加入番茄炒軟出水", "加入高麗菜大火快炒", "加鹽調味即可起鍋"] },
            { id: "R3", name: "蜂蜜烤地瓜", cal: 250, img: "images/蜂蜜烤地瓜.jpg", ingredients: ["地瓜"], steps: ["地瓜刷洗乾淨", "烤箱預熱 200 度", "放入地瓜烤 40 分鐘", "取出切開淋上蜂蜜"] },
            { id: "R4", name: "鳳梨蘋果汁", cal: 150, img: "images/鳳梨蘋果汁.jpg", ingredients: ["鳳梨", "蘋果"], steps: ["水果去皮切塊", "放入果汁機", "加入少許冰塊與水", "打成果汁即可飲用"] },
            { id: "R5", name: "香蕉柳橙冰沙", cal: 180, img: "images/香蕉柳橙冰沙.jpg", ingredients: ["香蕉", "柳橙"], steps: ["香蕉切片冷凍", "柳橙取果肉", "將所有食材放入調理機", "打成綿密冰沙狀"] },
            { id: "R6", name: "義式烤蔬菜", cal: 200, img: "images/義式烤蔬菜.jpg", ingredients: ["胡蘿蔔", "洋蔥"], steps: ["蔬菜切成滾刀塊", "淋上橄欖油與義式香料", "放入烤箱 200 度烤 25 分鐘"] }
        ];
        ...
        function showDetail(pid) {
            ...
            document.getElementById('dt-add-btn').onclick = function() { addToCart(p.id); };
            window.scrollTo(0,0);
        }
            ...
            list.forEach(r => {
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="btn-row">
                            <button class="btn-outline" onclick="showRecipeSteps('${r.id}')">查看做法</button>
        ...
        // 顯示步驟彈跳視窗 (修復的核心)
        function showRecipeSteps(rid) {
            const r = recipes.find(x => x.id === rid);
            if(!r) return;
            document.getElementById('steps-title').textContent = r.name;
            const stepsHtml = `<ol class="step-list">${r.steps.map(s => `<li>${s}</li>`).join('')}</ol>`;
        ...
        function addTempIngredient() { const v = document.getElementById('temp-ing-input').value.trim(); if(v) { tempIngredients.push(v); document.getElementById('temp-ing-input').value=''; updateCustomPreview(); }}
        function addTempStep() { const v = document.getElementById('temp-step-input').value.trim(); if(v) { tempSteps.push(v); document.getElementById('temp-step-input').value=''; updateCustomPreview(); }}
        function updateCustomPreview() {
            document.getElementById('new-r-ing-list').innerHTML = tempIngredients.length ? tempIngredients.map((item, i) => 
                `<div class="list-preview-item"><span>${item}</span><span style="color:red;cursor:pointer;" onclick="tempIngredients.splice(${i},1);updateCustomPreview()">✕</span></div>`).join('') : '尚未加入食材';
            document.getElementById('new-r-step-list').innerHTML = tempSteps.length ? tempSteps.map((item, i) => 
                `<div class="list-preview-item"><span>${i+1}. ${item}</span><span style="color:red;cursor:pointer;" onclick="tempSteps.splice(${i},1);updateCustomPreview()">✕</span></div>`).join('') : '尚未加入步驟';
        }
        function saveCustomRecipe() {
            const name = document.getElementById('new-r-name').value.trim();
            const cal = document.getElementById('new-r-cal').value;
            if(!name || tempIngredients.length===0 || tempSteps.length===0) { alert("請完整填寫名稱、食材與步驟！"); return; }
            recipes.unshift({ id: "C" + Date.now(), name: name, cal: cal||0, img: "https://via.placeholder.com/300?text=" + name, ingredients: [...tempIngredients], steps: [...tempSteps] });
            showToast("✨ 私房食譜發布成功！");
            switchTab('recipe');
        }
        ...
        function updateCart() {
            ...
            document.getElementById('cart-body').innerHTML = cart.length === 0 ? '<p style="text-align:center; color:#999;">購物車是空的</p>' : cart.map(item => `
        ...
        function updateBookModal() {
            document.getElementById('book-body').innerHTML = savedRecipes.length === 0 ? '<p style="text-align:center; color:#999;">尚未收藏</p>' : savedRecipes.map(rid => {
                const r = recipes.find(x => x.id === rid);
        ...
        function renderAdmin() {
            document.getElementById('admin-list').innerHTML = products.map(p => `<tr><td>${p.name}</td><td align="right"><button style="color:red; background:none;" onclick="alert('刪除功能演示')">刪除</button></td></tr>`).join('');
        }
